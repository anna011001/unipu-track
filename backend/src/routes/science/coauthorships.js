...
function validateRequiredYear(body, field, label, errors, required = true) {
  if (body[field] === undefined) {
    if (required) {
  ...
  if (!isIntegerInRange(body[field], 2000, maxYear)) {
    errors.push(`${label} mora biti između 2000. i ${maxYear}.`);
  }
}

function validateOptionalCount(body, field, label, errors) {
  if (body[field] === undefined || body[field] === null) {
    return;
  }

  if (!isIntegerInRange(body[field], 0, 9999)) {
    errors.push(`${label} mora biti cijeli broj između 0 i 9999.`);
  }
}
...
function validateYearTotal(body, mode = "create") {
  ...
  validateOptionalCount(body, "paper_count", "Broj radova", errors);
...
function validateCategorySummary(body, mode = "create") {
  ...
  if (!partial || body.category !== undefined) {
    if (!allowedCategories.includes(body.category)) {
      errors.push(`Kategorija mora biti: ${allowedCategories.join(", ")}.`);
    }
  }
  ...
  try {
    ...
    if (result.rows.length === 0) {
      return res.status(404).json({
        message: "Godišnji podatak o koautorstvu nije pronađen.",
      });
    }
...
router.post("/year-totals", async (req, res, next) => {
  const errors = validateYearTotal(req.body, "create");
...
router.put("/year-totals/:id", validateId, async (req, res, next) => {
  const errors = validateYearTotal(req.body, "put");
...
router.patch("/year-totals/:id", validateId, async (req, res, next) => {
  ...
  const errors = validateYearTotal(req.body, "patch");
...
router.get("/papers/:id", validateId, async (req, res, next) => {
  try {
    ...
    if (result.rows.length === 0) {
      return res.status(404).json({
        message: "Koautorski rad nije pronađen.",
      });
    }
...
router.get("/category-summaries/:id", validateId, async (req, res, next) => {
  try {
    ...
    if (result.rows.length === 0) {
      return res.status(404).json({
        message: "Sažetak kategorije nije pronađen.",
      });
    }
...
router.post("/category-summaries", async (req, res, next) => {
  const errors = validateCategorySummary(req.body, "create");
...
router.put("/category-summaries/:id", validateId, async (req, res, next) => {
  const errors = validateCategorySummary(req.body, "put");
...
router.patch("/category-summaries/:id", validateId, async (req, res, next) => {
  const allowedFields = [
    "reporting_period_id",
    "category",
    "calendar_year",
  ...
  const errors = validateCategorySummary(req.body, "patch");
