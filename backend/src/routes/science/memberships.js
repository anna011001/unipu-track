...
const organizationKinds = ["SCIENTIFIC", "PROFESSIONAL", "ARTISTIC"];

const organizationLevels = ["INTERNATIONAL", "NATIONAL", "REGIONAL"];
...
function validateRequiredId(body, field, label, errors, partial = false) {
...
function validateNewMembership(body, partial = false) {
  ...
  validateNullableId(body, "organization_id", "ID organizacije", errors);
  ...
  if (!partial || body.organization_name !== undefined) {
    ...
    } else if (body.organization_name.trim().length > 200) {
  ...
  if (
    ...
    errors.push("Datum učlanjenja mora biti u formatu YYYY-MM-DD.");
  }

  validateOptionalText(body, "membership_type", "Vrsta članstva", 40, errors);

  if (body.annual_fee_eur !== undefined && body.annual_fee_eur !== null) {
  ...
  validateNullableId(body, "unipu_member_id", "ID člana Sveučilišta", errors);
...
function validateActiveMembership(body, partial = false) {
  ...
  if (body.joined_year !== undefined && body.joined_year !== null) {
    const year = Number(body.joined_year);
    const currentYear = new Date().getFullYear();

    if (!Number.isInteger(year) || year < 1950 || year > currentYear) {
      errors.push(`Godina učlanjenja mora biti između 1950. i ${currentYear}.`);
    }
  }
...
function validateMembershipSummary(body, partial = false) {
  ...
  if (!partial || body.category_name !== undefined) {
    if (
      ...
      errors.push("Naziv kategorije smije imati najviše 100 znakova.");
    }
  }
  ...
  for (const field of countFields) {
    if (body[field] !== undefined) {
      ...
      if (!Number.isInteger(value) || value < 0 || value > 9999) {
        errors.push(`${field} mora biti cijeli broj između 0 i 9999.`);
      }
    }
  }

  if (body.share_percent !== undefined && body.share_percent !== null) {
    const value = Number(body.share_percent);

    if (!Number.isFinite(value) || value < 0 || value > 100) {
      errors.push("Postotak udjela mora biti između 0 i 100.");
    }
  }
...
function normalizeValue(field, value) {
  ...
  const numericFields = ["annual_fee_eur", "share_percent"];
...
async function patchRecord({ table, allowedFields, body, id }) {
  ...
  try {
    const result = await pool.query(
      ...
      [
        ...
        organization_id === null ? null : Number(organization_id),
        ...
        annual_fee_eur === null ? null : Number(annual_fee_eur),
        unipu_member_id === null ? null : Number(unipu_member_id),
