...
function validate() {
  ...
  if (form.total_project_amount_eur !== '' && (Number(form.total_project_amount_eur) < 0 || Number(form.total_project_amount_eur) > 999999.99)) result.push('Ukupni iznos projekta mora biti između 0 i 999999,99 eura.')
  if (form.unipu_share_eur !== '' && (Number(form.unipu_share_eur) < 0 || Number(form.unipu_share_eur) > 999999.99)) result.push('Udio UNIPU-a mora biti između 0 i 999999,99 eura.')
  return result
}
...
async function submit() {
  ...
  try {
    ...
    errors.value = exception.response?.data?.errors || [exception.response?.data?.message || 'Projektnu prijavu nije moguće spremiti.']
  } finally {
    saving.value = false
  }
}
...
<template>
  <main class="new-view">
    <nav>
      ...
      <RouterLink to="/istrazivanje-i-razvoj/projektne-prijave">Projektne prijave</RouterLink><span>›</span>
    ...
    <form v-else @submit.prevent="submit">
      ...
      <div class="card">
        <label>Naziv projektnog prijedloga *<textarea v-model="form.proposal_name" rows="3" :disabled="saved"></textarea></label>

        <div class="grid">
          <label>Izvor financiranja<input v-model="form.funding_source" :disabled="saved"></label>
          <label>Uloga UNIPU-a<input v-model="form.unipu_role" placeholder="Vodeći partner, nositelj projekta ili partner" :disabled="saved"></label>
          <label>Poziv / natječaj<input v-model="form.call_name" :disabled="saved"></label>
          <label>Poveznica na poziv<input v-model="form.call_link" type="url" placeholder="https://..." :disabled="saved"></label>
        </div>

        <div class="grid">
          <label>Uključene sastavnice<textarea v-model="form.involved_units" rows="3" :disabled="saved"></textarea></label>
          <label>Partnerske institucije<textarea v-model="form.partner_institutions" rows="3" :disabled="saved"></textarea></label>
        </div>

        <div class="grid">
          <label>Ukupni iznos projekta (€)<input v-model="form.total_project_amount_eur" type="number" min="0" max="999999.99" step="0.01" :disabled="saved"></label>
          <label>Udio UNIPU-a (€)<input v-model="form.unipu_share_eur" type="number" min="0" max="999999.99" step="0.01" :disabled="saved"></label>
          <label>Trajanje provedbe projekta<input v-model="form.implementation_duration" placeholder="npr. 24 mjeseca" :disabled="saved"></label>
          <label>Vrsta projekta<select v-model="form.project_type" :disabled="saved"><option value="">Odaberite vrstu</option><option v-for="item in projectTypes" :key="item.value" :value="item.value">{{ item.label }}</option></select></label>
        </div>

        <div class="grid">
          <label>Planirane aktivnosti<textarea v-model="form.planned_activities" rows="4" :disabled="saved"></textarea></label>
          <label>Projektni tim UNIPU-a<textarea v-model="form.unipu_project_team" rows="4" :disabled="saved"></textarea></label>
        </div>

        <div class="grid">
          <label>Rok za prijavu<input v-model="form.submission_deadline" type="date" :disabled="saved"></label>
          <label>Status prijave<select v-model="form.application_status" :disabled="saved"><option value="">Odaberite status</option><option v-for="item in statuses" :key="item.value" :value="item.value">{{ item.label }}</option></select></label>
          <label>Ugovor ili sporazum o partnerstvu<input v-model="form.contract_or_partnership_reference" placeholder="Klasa i urudžbeni broj" :disabled="saved"></label>
          <label>Šifra projekta po ugovoru<input v-model="form.contract_project_code" :disabled="saved"></label>
        </div>
      ...
      <button type="button" class="button back" @click="router.push('/istrazivanje-i-razvoj/projektne-prijave')">Natrag</button>
    </form>
