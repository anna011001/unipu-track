...
function validateForm() {
  ...
  if (form.program_name.trim().length > 250)
    errors.push('Naziv programa smije imati najviše 250 znakova.')
...
async function loadOptions() {
  ...
  try {
    ...
    if (!form.reporting_period_id) {
      errorMessages.value = [
        'Trenutno otvoreno izvještajno razdoblje nije pronađeno. Novo usavršavanje nije moguće spremiti.',
      ]
    }
  } catch (error) {
    errorMessages.value = [
...
<template>
  <main class="new-development-view">
    <nav class="breadcrumbs" aria-label="Putanja stranice">
      ...
      <RouterLink to="/istrazivanje-i-razvoj/strucna-usavrsavanja">Stručna usavršavanja</RouterLink
      ><span>›</span>
    ...
    <form v-else class="development-form" @submit.prevent="submitForm">
      ...
      <div class="form-card">
        <div class="form-grid two-columns">
          <label class="form-field"
            ...
            ><select v-model="form.staff_member_id" :disabled="saved">
          ...
          <label class="form-field"
            ...
            ><select v-model="form.organizational_unit_id" :disabled="saved">
        ...
        <div class="form-grid two-columns">
          <label class="form-field"
            ><span>Vrsta usavršavanja *</span
            ><select v-model="form.development_type" :disabled="saved">
              <option value="" disabled>Odaberite vrstu</option>
              <option value="STUDY_VISIT">Studijski boravak</option>
              <option value="WORKSHOP">Radionica</option>
              <option value="CONFERENCE">Konferencija</option>
              <option value="COURSE_CERTIFICATE">Tečaj ili certifikat</option>
              <option value="SUMMER_SCHOOL">Ljetna škola</option>
            </select></label
          >
          <label class="form-field"
            ...
            ><input
              ...
              maxlength="250"
              placeholder="Naziv usavršavanja ili programa"
              :disabled="saved"
          /></label>
        </div>
        ...
        <div class="form-grid two-columns">
          <label class="form-field"
            ><span>Ustanova domaćin</span
            ><input
              v-model="form.host_organization_name"
              list="host-organizations"
              maxlength="200"
              placeholder="Naziv ustanove"
              :disabled="saved"
              @change="useHostOrganizationData"
            /><datalist id="host-organizations">
        ...
        <div class="form-grid two-columns">
          <label class="form-field"
            ...
            ><input v-model="form.start_date" type="date" :disabled="saved"
          /></label>
          <label class="form-field"
            ><span>Završni datum</span><input v-model="form.end_date" type="date" :disabled="saved"
          /></label>
        </div>

        <label class="form-field wide-field"
          ><span>Medijska poveznica</span
          ><input
      ...
      <button
        ...
        type="button"
        @click="router.push('/istrazivanje-i-razvoj/strucna-usavrsavanja')"
      >
        Natrag
      </button>
    </form>
