...
function apiError(exception, fallback) {
  ...
  return Array.isArray(responseErrors)
    ? responseErrors
...
function detailPayload() {
  ...
  return Object.fromEntries(
    Object.entries(detail).map(([key, value]) => [
      key,
      numberFields.has(key) ? optionalNumber(value) : optionalText(value),
    ]),
  )
}
...
function validate() {
  ...
  if (statistics.value.some((row) => !row.country_name.trim()))
    result.push('Država je obavezna za svaki redak strukture sudionika.')
  const numericValues = [
    form.total_participants,
    form.foreign_participants,
    form.country_count,
    form.presentation_count,
    form.published_paper_count,
    detail.submitted_abstract_count,
    detail.accepted_abstract_count,
    detail.plenary_lecture_count,
  ...
  if (
    ...
    result.push('Brojčana polja moraju sadržavati nenegativne cijele brojeve.')
  if (
    form.total_participants !== '' &&
    form.foreign_participants !== '' &&
    Number(form.foreign_participants) > Number(form.total_participants)
  ) {
    result.push('Broj stranih sudionika ne može biti veći od ukupnog broja sudionika.')
  }
  if (detail.organization_cost_eur !== '' && Number(detail.organization_cost_eur) < 0)
    result.push('Trošak organizacije ne može biti negativan.')
  return result
}
...
<template>
  <main class="new-view">
    <nav>
      ...
      ><RouterLink to="/medunarodna-suradnja/medunarodne-konferencije"
        >Međunarodne konferencije</RouterLink
      ><span>›</span><span>Nova konferencija</span>
    </nav>
    ...
    <form v-else @submit.prevent="submit">
      ...
      <div class="card">
        <section>
          ...
          <div class="grid">
            <label class="wide"
              >Naziv konferencije *<input
                v-model="form.conference_name"
                maxlength="250"
                :disabled="saved"
            /></label>
            <label
              >Datum održavanja<input v-model="form.held_on" type="date" :disabled="saved"
            /></label>
            <label>Mjesto<input v-model="form.location" maxlength="150" :disabled="saved" /></label>
            <label
              >Organizator (sastavnica)<select v-model="form.organizer_unit_id" :disabled="saved">
            ...
            >
            <label
              >Znanstveno područje<input
                v-model="form.scientific_field"
                maxlength="150"
                :disabled="saved"
            /></label>
            <label class="wide"
              >Suorganizatori<textarea
                v-model="form.coorganizers"
                rows="3"
                :disabled="saved"
              ></textarea>
            </label>
          ...
          <div class="grid counts">
            ...
            /></label>
            <label
              >Strani sudionici<input
                v-model="form.foreign_participants"
                type="number"
                min="0"
                :max="form.total_participants || 9999"
                :disabled="saved"
            /></label>
            <label
              >Broj država<input
                v-model="form.country_count"
                type="number"
                min="0"
                max="250"
                :disabled="saved || statistics.length > 0"
            /></label>
            <label
              >Broj izlaganja<input
                v-model="form.presentation_count"
                type="number"
                min="0"
                max="9999"
                :disabled="saved"
            /></label>
            <label
              >Objavljeni radovi<input
                v-model="form.published_paper_count"
                type="number"
                min="0"
                max="9999"
                :disabled="saved"
          ...
          <div class="grid">
            <label class="wide"
              >Link na web / zbornik<input
                v-model="form.web_or_proceedings_link"
                type="url"
                :disabled="saved"
            /></label>
            <label class="wide"
        ...
        <section>
          ...
          <div class="grid">
            <label class="wide"
              >Puni naziv na engleskom<input
                v-model="detail.english_name"
                maxlength="250"
                :disabled="saved"
            /></label>
            <label
              >Predsjednik organizacijskog odbora<input
                v-model="detail.organizing_committee_chair"
                maxlength="120"
                :disabled="saved"
            /></label>
            <label
              >Predsjednik programskog odbora<input
                v-model="detail.program_committee_chair"
                maxlength="120"
                :disabled="saved"
            /></label>
            <label
              >Članovi programskog odbora – UNIPU<textarea
                v-model="detail.unipu_program_members"
                rows="3"
                :disabled="saved"
              ></textarea>
            </label>
            <label
              >Strani članovi programskog odbora<textarea
                v-model="detail.foreign_program_members"
                rows="3"
                :disabled="saved"
              ></textarea>
            </label>
          ...
          <div class="grid counts detail-counts">
            <label
              >Prijavljeni sažeci<input
                v-model="detail.submitted_abstract_count"
                type="number"
                min="0"
                max="9999"
                :disabled="saved"
            /></label>
            <label
              >Prihvaćeni sažeci<input
                v-model="detail.accepted_abstract_count"
                type="number"
                min="0"
                max="9999"
                :disabled="saved"
            /></label>
            <label
              >Plenarna predavanja<input
                v-model="detail.plenary_lecture_count"
                type="number"
                min="0"
                max="999"
                :disabled="saved"
          ...
          <div class="grid">
            <label
              >Indeksacija zbornika<input
                v-model="detail.proceedings_indexing"
                maxlength="150"
                :disabled="saved"
            /></label>
            <label
              >Web stranica konferencije<input
                v-model="detail.conference_website"
                type="url"
                :disabled="saved"
            /></label>
            <label
              >Ukupni troškovi organizacije (€)<input
                v-model="detail.organization_cost_eur"
                type="number"
                min="0"
                max="999999.99"
                step="0.01"
            ...
            /></label>
            <label
              >Izvori financiranja<input v-model="detail.funding_sources" :disabled="saved"
            /></label>
            <label class="wide"
              >Medijska pokrivenost<textarea
                v-model="detail.media_coverage"
                rows="3"
                :disabled="saved"
              ></textarea>
            </label>
        ...
        <section>
          <header class="section-heading">
            <h2>Struktura sudionika po državama</h2>
            <button
              v-if="!saved"
              type="button"
              class="square plus"
              aria-label="Dodaj državu"
              @click="addStatistic"
            >
              +
            </button>
          </header>
          <div v-if="statistics.length" class="table">
            <table>
              <thead>
                <tr>
                  <th>Država</th>
              ...
              <tbody>
                <tr v-for="row in statistics" :key="row.temporaryId">
                  <td>
                    <CountryAutocomplete
                  ...
                  <td>
                    <button
                      ...
                      type="button"
                      class="square minus"
                      aria-label="Ukloni državu"
                      @click="removeStatistic(row)"
                    >
                      −
                    </button>
                  </td>
      ...
      <button
        type="button"
        class="button back"
        @click="router.push('/medunarodna-suradnja/medunarodne-konferencije')"
      >
        Natrag
      </button>
    </form>
