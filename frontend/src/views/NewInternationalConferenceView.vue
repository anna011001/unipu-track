    ...
    <form v-else @submit.prevent="submit">
      ...
      <div class="card">
        ...
        <section>
          ...
          <div v-if="statistics.length" class="table"><table><thead><tr><th>Država</th><th>Broj sudionika</th><th>Broj izlaganja</th><th></th></tr></thead><tbody><tr v-for="row in statistics" :key="row.temporaryId"><td><select v-model="row.country_id" :disabled="saved" @change="updateCountryName(row)"><option value="">Druga država</option><option v-for="country in countries" :key="country.id" :value="country.id">{{ country.name_hr || country.name_en }}</option></select><input v-if="!row.country_id" v-model="row.country_name" class="custom-country" maxlength="100" placeholder="Naziv države" :disabled="saved"></td><td><input v-model="row.participant_count" type="number" min="0" max="9999" :disabled="saved"></td><td><input v-model="row.presentation_count" type="number" min="0" max="9999" :disabled="saved"></td><td><button v-if="!saved" type="button" class="square minus" aria-label="Ukloni državu" @click="removeStatistic(row)">−</button></td></tr></tbody></table></div>
        </section>
