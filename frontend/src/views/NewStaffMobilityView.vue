...
const selectedStaff = computed(() => staff.value.find((item) => Number(item.id) === Number(form.staff_member_id)))
...
function validate() {
  ...
  if (form.start_date && form.end_date && form.end_date < form.start_date) result.push('Datum završetka ne smije biti prije datuma početka.')
...
async function submit() {
  ...
  try {
    await api.post('/api/staff-mobilities', {
      ...
      program_name: form.program_name === 'Ostalo' ? optionalText(form.other_program_name) || 'Ostalo' : optionalText(form.program_name),
  ...
  } catch (exception) { errors.value = apiErrors(exception, 'Mobilnost nije moguće spremiti.') } finally { saving.value = false }
}

async function load() {
  try {
    const [periodResponse, staffResponse, unitResponse, countryResponse] = await Promise.all([
      api.get('/api/reporting-periods'), api.get('/api/staff-members'), api.get('/api/organizational-units'), api.get('/api/countries'),
    ])
...
watch(() => form.staff_member_id, () => {
  if (selectedStaff.value?.organizational_unit_id) form.organizational_unit_id = selectedStaff.value.organizational_unit_id
})
watch([() => form.start_date, () => form.end_date], () => {
  if (!form.start_date || !form.end_date || form.end_date < form.start_date) return
  const days = Math.round((new Date(`${form.end_date}T00:00:00`) - new Date(`${form.start_date}T00:00:00`)) / 86400000) + 1
...
<template>
  <main class="new-view">
    <nav><RouterLink to="/medunarodna-suradnja">Međunarodna suradnja</RouterLink><span>›</span><RouterLink to="/medunarodna-suradnja/mobilnost-osoblja">Mobilnost osoblja</RouterLink><span>›</span><span>Nova mobilnost</span></nav>
    ...
    <form v-else @submit.prevent="submit">
      ...
      <div class="card">
        <section>
          ...
          <div class="grid">
            <label>Ime i prezime *<select v-model="form.staff_member_id" :disabled="saved"><option value="">Odaberite djelatnika</option><option v-for="member in staff" :key="member.id" :value="member.id">{{ member.first_name }} {{ member.last_name }}</option></select></label>
            ...
            <label>Vrsta mobilnosti *<select v-model="form.mobility_type" :disabled="saved"><option value="">Odaberite vrstu</option><option>Nastava</option><option>Stručno usavršavanje</option><option>Istraživački boravak</option><option>Ostalo</option></select></label>
        ...
        <section>
          ...
          <div class="grid three">
            ...
            <label>Datum završetka<input v-model="form.end_date" type="date" :disabled="saved"></label>
        ...
        <section>
          ...
          <label>Svrha mobilnosti<textarea v-model="form.mobility_purpose" rows="3" :disabled="saved"></textarea></label>
          <div class="grid">
            <label>Aktivnosti<textarea v-model="form.activities" rows="4" :disabled="saved"></textarea></label>
            <label>Rezultati<textarea v-model="form.results" rows="4" :disabled="saved"></textarea></label>
          </div>
      ...
      <button type="button" class="button back" @click="router.push('/medunarodna-suradnja/mobilnost-osoblja')">Natrag</button>
    </form>
