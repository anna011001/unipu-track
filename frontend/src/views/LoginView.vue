...
async function register() {
  ...
  if (
    ...
    !registrationForm.academic_title ||
    !registrationForm.organizational_unit_id ||
  ...
  try {
    ...
    error.value = Array.isArray(details)
      ? details.join(' ')
      : exception.response?.data?.message || 'Registracija trenutačno nije moguća.'
  } finally {
    submitting.value = false
  }
}
  ...
  <main class="login-view">
    ...
    <section class="login-content">
      ...
      <form v-if="mode === 'login'" class="login-card" @submit.prevent="login">
        ...
        <button class="secondary-button" type="button" @click="changeMode('register')">
          Registrirajte se
        </button>
      </form>
      ...
      <form v-else class="login-card registration-card" @submit.prevent="register">
        ...
        <div class="name-fields">
          <label>
            ...
            <select v-model="registrationForm.academic_title" :disabled="submitting">
              ...
              <option v-for="title in academicTitles" :key="title" :value="title">
                {{ title }}
              </option>
            </select>
          </label>
        ...
        <label>
          ...
          <span class="password-field">
            <input
              ...
              :disabled="submitting"
            />
            <button type="button" @click="showRegistrationPassword = !showRegistrationPassword">
              {{ showRegistrationPassword ? 'Sakrij' : 'Prikaži' }}
            </button>
          </span>
        </label>
        ...
        <button class="secondary-button" type="button" @click="changeMode('login')">
          Natrag na prijavu
        </button>
      </form>
    </section>
...
.login-view {
  min-height: 100vh;
  padding: 24px clamp(24px, 4vw, 72px) 48px;
