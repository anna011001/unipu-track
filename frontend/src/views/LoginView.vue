...
const registrationForm = reactive({ first_name: '', last_name: '', email: '', academic_title: '', organizational_unit_id: '', password: '', password_confirmation: '' })
const academicTitles = ref([])
...
async function loadRegistrationOptions() {
  try {
    const response = await api.get('/api/auth/registration-options')
    academicTitles.value = response.data.academic_titles || []
    organizationalUnits.value = response.data.organizational_units || []
  } catch {
    error.value = 'Podatke za registraciju trenutačno nije moguće učitati.'
  }
}
...
async function register() {
  ...
  if (!registrationForm.first_name.trim() || !registrationForm.last_name.trim() || !registrationForm.email.trim() || !registrationForm.academic_title || !registrationForm.organizational_unit_id || !registrationForm.password || !registrationForm.password_confirmation) {
  ...
  try {
    const response = await api.post('/api/auth/register', {
      ...
      academic_title: registrationForm.academic_title,
      organizational_unit_id: registrationForm.organizational_unit_id,
  ...
  <main class="login-view">
    ...
    <section class="login-content">
      ...
      <form v-if="mode === 'login'" class="login-card" @submit.prevent="login">
        ...
        <label>
          <span>E-mail</span>
          <input v-model="form.email" name="username" type="email" autocomplete="username" placeholder="ime.prezime@unipu.hr" :disabled="submitting">
        </label>

        <label>
          <span>Lozinka</span>
          <span class="password-field">
            <input v-model="form.password" name="password" :type="showPassword ? 'text' : 'password'" autocomplete="current-password" placeholder="Unesite lozinku" :disabled="submitting">
      ...
      <form v-else class="login-card registration-card" @submit.prevent="register">
        ...
        <label>
          ...
          <input v-model="registrationForm.email" name="email" type="email" autocomplete="email" placeholder="ime.prezime@unipu.hr" :disabled="submitting">
        </label>

        <div class="name-fields">
          <label>
            <span>Zvanje</span>
            <select v-model="registrationForm.academic_title" :disabled="submitting">
              <option value="">Odaberite zvanje</option>
              <option v-for="title in academicTitles" :key="title" :value="title">{{ title }}</option>
            </select>
          </label>
          <label>
            <span>Sastavnica</span>
            <select v-model="registrationForm.organizational_unit_id" :disabled="submitting">
              <option value="">Odaberite sastavnicu</option>
              <option v-for="unit in organizationalUnits" :key="unit.id" :value="unit.id">{{ unit.short_name }}</option>
            </select>
          </label>
        </div>
        ...
        <label>
          <span>Lozinka</span>
          <span class="password-field">
            <input v-model="registrationForm.password" name="new-password" :type="showRegistrationPassword ? 'text' : 'password'" autocomplete="new-password" placeholder="Najmanje 8 znakova" :disabled="submitting">
        ...
        <label>
          ...
          <input v-model="registrationForm.password_confirmation" name="new-password-confirmation" type="password" autocomplete="new-password" :disabled="submitting">
        </label>
