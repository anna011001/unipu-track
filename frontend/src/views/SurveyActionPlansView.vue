...
const filteredPlans = computed(() =>
  plans.value.filter((item) => {
    const belongsToPeriod =
      !periodId.value || Number(item.reporting_period_id) === Number(periodId.value)
    return belongsToPeriod && isSurveyActionPlan(item)
  }),
)
const selectedPlan = computed(() =>
  plans.value.find((item) => Number(item.id) === Number(selectedId.value)),
)
const selectedStaff = computed(() =>
  staff.value.find((item) => Number(item.id) === Number(staffMemberId.value)),
)
const reportValues = computed(() =>
  Object.fromEntries(columns.map((column) => [column.key, combinedValues(column.key)])),
)
...
async function remove() {
  if (
    !selectedPlan.value ||
    !confirm(`Želite li izbrisati izvješće za „${staffName(selectedPlan.value.staff_member_id)}”?`)
  )
    return
  saving.value = true
  try {
...
<template>
  <main class="view">
    <nav class="breadcrumbs">
      ...
      ><span>Studentske ankete</span>
    </nav>
    <h1>Planiranje mjera nakon vrednovanja nastavnog rada studentskim anketama</h1>
    <p class="intro">
      Odaberite nastavnika i jednu ili više mogućnosti u svakom stupcu. Izvješće se sastavlja
      automatski ispod tablice.
    </p>
    ...
    <template v-if="!loading">
      <div class="topbar">
        ...
        <div class="toolbar">
          ...
          <button :disabled="saving" @click="save">
            {{ selectedId ? 'Spremi izmjene' : 'Spremi izvješće' }}
          </button>
      ...
      <section class="planner-wrap" aria-label="Plan mjera nakon studentskih anketa">
        <div class="planner-table">
          ...
          <div v-for="column in columns" :key="column.key" class="table-heading">
            {{ column.label }}
          </div>
          ...
          <div class="selection-cell teacher-cell">
            ...
            <p v-if="selectedStaff">
              <strong>{{ selectedStaff.academic_title || 'Bez zvanja' }}</strong
              ><br />{{
                selectedStaff.organizational_unit_short_name ||
                selectedStaff.organizational_unit_name ||
                ''
              }}
            </p>
          </div>
          ...
          <div
            ...
            :key="`${column.key}-options`"
            class="selection-cell option-groups"
          >
            <div
              v-for="(group, groupIndex) in optionGroups(column)"
              :key="`${column.key}-${groupIndex}`"
              class="option-group"
            >
              <label
                ...
                :key="option"
                class="choice"
                :class="{ chosen: selections[column.key]?.includes(option) }"
              >
        ...
        <div class="saved-table-wrap">
          <table>
            <thead>
              <tr>
                <th>Nastavnik</th>
                <th>Razlog uvrštenja</th>
                <th>Uočeni nedostatak</th>
                <th>Mjere unaprjeđenja</th>
            ...
            <tbody>
              <tr
                ...
                :key="plan.id"
                :class="{ active: selectedId === plan.id }"
                @click="loadPlan(plan)"
              >
                <td>{{ staffName(plan.staff_member_id) }}</td>
                <td>{{ plan.inclusion_reasons }}</td>
                <td>{{ plan.observed_deficiency || '—' }}</td>
                <td>{{ plan.improvement_measures || '—' }}</td>
                <td>{{ plan.target_value || '—' }}</td>
              </tr>
              <tr v-if="!filteredPlans.length">
                <td colspan="5" class="empty">Za odabrano razdoblje nema spremljenih izvješća.</td>
              </tr>
            </tbody>
          </table>
        </div>
...
.view {
  ...
  padding: 34px clamp(24px, 4vw, 90px) 90px;
...
.planner-table {
  display: grid;
  grid-template-columns: 220px repeat(5, minmax(245px, 1fr));
  min-width: 1460px;
...
.table-heading {
  border-right: 1px solid rgba(var(--v-theme-on-primary), 0.55);
}
.table-heading:nth-child(6) {
  border-right: 0;
}
.selection-cell.option-groups {
  display: grid;
...
.option-group:first-child {
  border-bottom: 3px solid rgb(var(--v-theme-category-border));
}
.option-group .choice {
  min-height: 0;
  flex: 1;
}
.option-group .choice:last-child {
  border-bottom: 0;
}
</style>
