...
function currentPeriod(items) {
  ...
  return items.filter((period) => !period.is_closed && String(period.start_date).slice(0, 10) <= today && today <= String(period.end_date).slice(0, 10)).sort((a, b) => String(b.start_date).localeCompare(String(a.start_date)))[0]
}
...
function apiErrors(exception, fallback) { return exception.response?.data?.errors || [exception.response?.data?.message || fallback] }
...
function validate() {
  ...
  if (isAgreement.value && form.signed_on && form.valid_until && form.valid_until < form.signed_on) result.push('Rok važenja ne smije biti prije datuma potpisivanja.')
  return result
}
...
async function submit() {
  ...
  try {
    ...
    saved.value = true
  } catch (exception) { errors.value = apiErrors(exception, 'Zapis nije moguće spremiti.') } finally { saving.value = false }
}

async function load() {
  try {
    const [periodResponse, countryResponse, unitResponse] = await Promise.all([
      api.get('/api/reporting-periods'), api.get('/api/countries'), api.get('/api/organizational-units'),
    ])
    periods.value = periodResponse.data; countries.value = countryResponse.data; units.value = unitResponse.data
    form.reporting_period_id = currentPeriod(periods.value)?.id ?? periods.value.find((period) => !period.is_closed)?.id ?? null
  } catch (exception) { errors.value = apiErrors(exception, 'Nije moguće učitati obrazac.') } finally { loading.value = false }
}
...
<template>
  <main class="new-view">
    <nav><RouterLink to="/medunarodna-suradnja">Međunarodna suradnja</RouterLink><span>›</span><RouterLink to="/medunarodna-suradnja/partnerstva">Partnerstva</RouterLink><span>›</span><span>{{ isAgreement ? 'Novi ugovor' : 'Novo partnerstvo' }}</span></nav>
    <h1>{{ isAgreement ? 'Dodavanje aktivnog međunarodnog ugovora' : 'Dodavanje novog međunarodnog partnerstva' }}</h1>
    ...
    <form v-else @submit.prevent="submit">
      ...
      <div class="card">
        ...
        <section v-if="!isAgreement">
          ...
          <div class="grid">
            <label>Područje suradnje<input v-model="form.cooperation_field" maxlength="150" :disabled="saved"></label>
            <label>Kontakt osoba UNIPU-a<input v-model="form.unipu_contact_person" maxlength="120" :disabled="saved"></label>
            <label>Datum početka<input v-model="form.start_date" type="date" :disabled="saved"></label>
            <label>Trajanje<input v-model="form.duration" maxlength="80" placeholder="npr. 5 godina" :disabled="saved"></label>
          </div>
          <label>Poveznica na ugovor<input v-model="form.agreement_link" type="url" placeholder="https://" :disabled="saved"></label>
        </section>

        <section v-else>
          <h2>Podaci o ugovoru</h2>
          <div class="grid">
            <label>Datum potpisivanja<input v-model="form.signed_on" type="date" :disabled="saved"></label>
            <label>Rok važenja<input v-model="form.valid_until" type="date" :disabled="saved"></label>
            <label>Odgovorna osoba<input v-model="form.responsible_person" maxlength="120" :disabled="saved"></label>
            <label>Dokument<input v-model="form.document_link" type="url" placeholder="https://" :disabled="saved"></label>
          </div>
          <label>Realizirane aktivnosti<textarea v-model="form.completed_activities" rows="4" :disabled="saved"></textarea></label>
        </section>
        ...
        <section>
          ...
          <label>Napomena<textarea v-model="form.notes" rows="3" :disabled="saved"></textarea></label>
        </section>
        <div class="actions"><button class="button" :disabled="saving || saved">{{ saving ? 'Spremanje...' : 'Spremi' }}</button></div>
      </div>
      <button type="button" class="button back" @click="router.push('/medunarodna-suradnja/partnerstva')">Natrag</button>
    </form>
