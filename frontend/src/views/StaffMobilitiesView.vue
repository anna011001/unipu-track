...
<template>
  <main class="view">
    ...
    <template v-else>
      <section class="overview"><label>Izvještajno razdoblje<select v-model.number="periodId"><option v-for="period in periods" :key="period.id" :value="period.id">{{ period.label }}</option></select></label><dl class="metrics"><div><dt>Ukupno mobilnosti</dt><dd>{{ filtered.length }}</dd></div><div><dt>Osoba u mobilnosti</dt><dd>{{ peopleCount }}</dd></div><div><dt>Prosječno po osobi</dt><dd>{{ averagePerPerson }}</dd></div></dl><ExportButton :records="filtered" file-name="mobilnost-osoblja" /></section>
