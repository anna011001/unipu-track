...
const filtered = computed(() =>
  records.value.filter((item) => Number(item.reporting_period_id) === Number(periodId.value)),
)
...
const averagePerPerson = computed(() =>
  peopleCount.value
    ? (filtered.value.length / peopleCount.value).toFixed(2).replace('.', ',')
    : '0',
)
...
function staffName(id) {
  const item = staff.value.find((member) => Number(member.id) === Number(id))
...
function value(field) {
  ...
  if (field.kind === 'staff') return staffName(raw)
...
function payload() {
  ...
  for (const field of fields) {
    ...
    if (field.kind === 'program')
      result[field.key] =
        raw === 'Ostalo'
          ? optionalText(form.value.custom_program_name) || 'Ostalo'
          : optionalText(raw)
    else if (field.required && field.kind !== 'staff') result[field.key] = String(raw ?? '').trim()
    else if (['staff', 'unit', 'country', 'number'].includes(field.kind))
      result[field.key] = optionalNumber(raw)
...
async function save() {
  editError.value = ''
  if (!form.value.staff_member_id || !String(form.value.mobility_type).trim()) {
    editError.value = 'Djelatnik i vrsta mobilnosti su obavezni.'
    return
  }
  if (form.value.start_date && form.value.end_date && form.value.end_date < form.value.start_date) {
    editError.value = 'Datum završetka ne smije biti prije datuma početka.'
    return
  }
  saving.value = true
  try {
    const response = await api.patch(`/api/staff-mobilities/${selected.value.id}`, payload())
    const staffMember = staff.value.find(
      (item) => Number(item.id) === Number(response.data.staff_member_id),
    )
    const updated = {
      ...selected.value,
      ...response.data,
      staff_first_name: staffMember?.first_name,
      staff_last_name: staffMember?.last_name,
    }
    ...
    editError.value = apiError(exception, 'Mobilnost nije moguće izmijeniti.')
  } finally {
    saving.value = false
  }
}
...
async function remove() {
  ...
  try {
    ...
    editError.value = apiError(exception, 'Mobilnost nije moguće izbrisati.')
  } finally {
...
async function load() {
  try {
    const [periodResponse, recordResponse, staffResponse, unitResponse, countryResponse] =
      await Promise.all([
        api.get('/api/reporting-periods'),
        api.get('/api/staff-mobilities'),
    ...
    error.value = apiError(exception, 'Nije moguće dohvatiti mobilnosti osoblja.')
  } finally {
    loading.value = false
  }
}
...
watch(periodId, async () => {
  ...
  try {
    ...
    error.value = apiError(exception, 'Nije moguće dohvatiti analize mobilnosti.')
  }
})
onMounted(load)
onUnmounted(() => {
  ...
  <main class="view">
    ...
    <template v-else>
      <section class="overview">
        ...
        <dl class="metrics">
          <div>
            <dt>Ukupno mobilnosti</dt>
            <dd>{{ filtered.length }}</dd>
          </div>
          <div>
            <dt>Osoba u mobilnosti</dt>
            <dd>{{ peopleCount }}</dd>
          </div>
          <div>
            <dt>Prosječno po osobi</dt>
            <dd>{{ averagePerPerson }}</dd>
          </div>
        </dl>
        <ExportButton :records="filtered" file-name="mobilnost-osoblja" />
      </section>
      <section class="records">
        <header class="heading">
          <h2>Pojedinačne mobilnosti ({{ filtered.length }})</h2>
          <RouterLink class="action" to="/medunarodna-suradnja/mobilnost-osoblja/nova"
            >Dodaj mobilnost</RouterLink
          >
        </header>
        <div v-if="filtered.length" class="layout">
          <div>
            <div class="list">
              <button
                ...
                @click="choose(item)"
              >
                <strong>{{ item.staff_first_name }} {{ item.staff_last_name }}</strong
                ><span
                  >{{ item.host_institution || item.mobility_type }} ·
                  {{ date(item.start_date) }}</span
                >
              </button>
            </div>
          ...
          <div v-if="selected" ref="detailsCard" class="details">
            ...
            <dl>
              <div>
                <dt>Zvanje</dt>
                <dd>{{ selected.staff_academic_title || '—' }}</dd>
              </div>
              <div v-for="field in fields" :key="field.key">
                <dt>{{ field.label }}</dt>
                <dd>
                  <select
                    v-if="form && field.kind === 'staff'"
                  ...
                  ><CountryAutocomplete
                    v-else-if="form && field.kind === 'country'"
                    v-model="form[field.key]"
                    :countries="countries"
                    class="input"
                    placeholder="Nije odabrano"
                  /><template v-else-if="form && field.kind === 'program'"
                    ><select v-model="form[field.key]" class="input">
                      <option value="">Nije odabrano</option>
                      <option>Erasmus+</option>
                      <option>CEEPUS</option>
                      <option>Bilateralni</option>
                      <option>Ostalo</option></select
                    ><input
                      v-if="form[field.key] === 'Ostalo'"
                      v-model="form.custom_program_name"
                      class="input custom-program"
                      maxlength="100"
                      placeholder="Naziv drugog programa" /></template
                  ><textarea
                  ...
                  /><template v-else>{{ value(field) }}</template>
                </dd>
              </div>
            </dl>
          </div>
        </div>
        <p v-else>Nema mobilnosti za odabrano razdoblje.</p>
      </section>
          ...
          <table>
            <thead>
              <tr>
                ...
                <th>Osobe u mobilnosti</th>
                <th>Ukupno mobilnosti</th>
                <th>Prosječno po osobi</th>
                <th>Erasmus+ nastava</th>
                <th>Erasmus+ usavršavanje</th>
                <th>CEEPUS</th>
                <th>Bilateralni</th>
                <th>Ostalo</th>
                <th>Ukupno dana</th>
            ...
            <tbody>
              <tr v-for="row in unitAnalyses" :key="row.organizational_unit_id">
                <td>{{ row.organizational_unit_short_name || row.organizational_unit_name }}</td>
                <td>{{ row.employee_count }}</td>
                <td>{{ row.people_in_mobility_count }}</td>
                <td>{{ row.mobility_count }}</td>
                <td>{{ row.average_per_person }}</td>
                <td>{{ row.erasmus_teaching_count }}</td>
                <td>{{ row.erasmus_training_count }}</td>
                <td>{{ row.ceepus_count }}</td>
                <td>{{ row.bilateral_count }}</td>
      ...
      <section class="summary">
        <h2>Osoblje s višestrukim mobilnostima</h2>
        <div class="table">
          <table>
            <thead>
              <tr>
            ...
            <tbody>
              <tr v-for="row in multipleAnalyses" :key="row.staff_member_id">
                <td>{{ row.first_name }} {{ row.last_name }}</td>
                <td>
                  {{ row.organizational_unit_short_name || row.organizational_unit_name || '—' }}
                </td>
                <td>{{ row.mobility_count }}</td>
                <td>{{ row.total_days }}</td>
                <td>{{ row.countries || '—' }}</td>
              ...
              <tr v-if="!multipleAnalyses.length">
                <td colspan="6">Nema osoba s višestrukim mobilnostima.</td>
              </tr>
            </tbody>
          </table>
        </div>
      ...
      <section class="summary">
        <h2>Analiza po državama odredišta</h2>
          ...
          <table>
            <thead>
              <tr>
                ...
                <th>Broj osoba</th>
                <th>Ukupno dana</th>
                <th>Najčešći program</th>
              </tr>
            </thead>
            <tbody>
              <tr v-for="row in countryAnalyses" :key="row.country_id">
                <td>{{ row.country_name }}</td>
                <td>{{ row.mobility_count }}</td>
                <td>{{ row.people_count }}</td>
                <td>{{ row.total_days }}</td>
                <td>{{ row.most_common_program || '—' }}</td>
              </tr>
              <tr v-if="!countryAnalyses.length">
                <td colspan="5">Nema podataka po državama.</td>
              </tr>
            </tbody>
          </table>
        </div>
