...
function validate() {
  ...
  if (
    !Number.isInteger(Number(form.publication_year)) ||
    Number(form.publication_year) < 2000 ||
    Number(form.publication_year) > maxYear
  ) {
...
async function submit() {
  ...
  try {
    ...
    errors.value = error.response?.data?.errors || [
      error.response?.data?.message || 'Koautorski rad nije moguće spremiti.',
    ]
  } finally {
    saving.value = false
  }
...
<template>
  <main class="new-view">
    ...
    <form v-else @submit.prevent="submit">
      ...
      <div class="card">
        <label
          >Autori i naslov rada *<textarea
            v-model="form.authors_and_title"
            rows="5"
            :disabled="saved"
            placeholder="Prezime, I., Prezime, I. (2026). Naslov rada. Naziv časopisa..."
          ></textarea>
        </label>
        <div class="grid">
          <label
          ...
          /></label>
          <label
            >Kategorija rada *<select v-model="form.category" :disabled="saved">
        ...
        <label
          >Link na objavljeni rad<input
            v-model="form.publication_link"
            type="url"
      ...
      <button
        type="button"
        class="button back"
        @click="router.push('/istrazivanje-i-razvoj/koautorstva')"
      >
        Natrag
      </button>
    </form>
