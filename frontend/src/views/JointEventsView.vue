...
const eventTypes = ['Međunarodne konferencije', 'Domaće konferencije', 'Znanstveni skupovi', 'Stručni skupovi', 'Okrugli stolovi', 'Radionice']
const heldFields = [
...
const configs = {
  held: { title: 'Održana zajednička događanja', source: heldEvents, fields: heldFields, endpoint: 'held' },
  planned: { title: 'Planirana zajednička događanja', source: plannedEvents, fields: plannedFields, endpoint: 'planned' },
}
...
const typeAnalysis = computed(() => eventTypes.map((eventType) => {
  ...
  return { eventType, held: held.length, planned: planned.length, participants, average: held.length ? Math.round(participants / held.length) : 0 }
}))

function rows(type) { return configs[type].source.value.filter((item) => Number(item.reporting_period_id) === Number(periodId.value)) }
function selectedItem(type) { return activeType.value === type && activeId.value !== 'new' ? rows(type).find((item) => item.id === activeId.value) || null : null }
...
function formatMoney(value) { return value === null || value === undefined || value === '' ? '—' : new Intl.NumberFormat('hr-HR', { style: 'currency', currency: 'EUR' }).format(value) }
function display(field, value) { if (field.kind === 'country') return countryName(value); if (field.kind === 'date') return formatDate(value); if (field.kind === 'number') return formatMoney(value); if (field.kind === 'type') return eventTypes.includes(value) ? value : '—'; return value === null || value === undefined || value === '' ? '—' : value }
...
function add(type) { activeType.value = type; activeId.value = 'new'; form.value = emptyForm(type) }
function edit(type) {
  const item = rows(type).find((entry) => entry.id === activeId.value)
  if (!item) return
  form.value = Object.fromEntries(configs[type].fields.map((field) => [field.key, field.kind === 'date' ? String(item[field.key] || '').slice(0, 10) : item[field.key] ?? '']))
}
...
function payload(type) {
  ...
  for (const field of configs[type].fields) {
    ...
    data[field.key] = ['country', 'integer', 'number'].includes(field.kind) ? optionalNumber(value) : field.required ? String(value ?? '').trim() : optionalText(value)
  }
  return data
}
async function save(type) {
  if (!String(form.value.event_name || '').trim()) { error.value = 'Naziv događanja je obavezan.'; return }
  saving.value = true; error.value = ''
  try {
    const data = payload(type)
    const response = activeId.value === 'new'
      ? await api.post(`/api/joint-events/${configs[type].endpoint}`, { ...data, reporting_period_id: periodId.value, created_by: userId })
    ...
    if (index >= 0) source.value[index] = response.data; else source.value.unshift(response.data)
    activeType.value = type; activeId.value = response.data.id; form.value = null; toast('Događanje je uspješno spremljeno.')
  } catch (exception) { error.value = apiError(exception, 'Događanje nije moguće spremiti.') } finally { saving.value = false }
}
async function remove(type) {
  const item = rows(type).find((entry) => entry.id === activeId.value)
  if (!item || !confirm('Želite li izbrisati odabrano događanje?')) return
  try { await api.delete(`/api/joint-events/${configs[type].endpoint}/${item.id}`); configs[type].source.value = configs[type].source.value.filter((entry) => entry.id !== item.id); resetSelection(); toast('Događanje je izbrisano.') } catch (exception) { error.value = apiError(exception, 'Događanje nije moguće izbrisati.') }
}
function selectFromRoute() {
  ...
  if (item) { periodId.value = item.reporting_period_id; activeType.value = type; activeId.value = id }
}
async function load() {
  try {
    const responses = await Promise.all([api.get('/api/reporting-periods'), api.get('/api/countries'), api.get('/api/joint-events/held'), api.get('/api/joint-events/planned')])
    ;[periods.value, countries.value, heldEvents.value, plannedEvents.value] = responses.map((response) => response.data)
    ...
    selectFromRoute()
  } catch (exception) { error.value = apiError(exception, 'Podatke o zajedničkim događanjima nije moguće dohvatiti.') } finally { loading.value = false }
}
watch(periodId, (value, previousValue) => { if (previousValue !== null) resetSelection() })
...
<template>
  <main class="page">
    <nav class="breadcrumbs"><RouterLink to="/suradnja-i-dogadanja">Suradnja i događanja</RouterLink><span>›</span><span>Zajednička događanja</span></nav>
    ...
    <template v-if="!loading">
      ...
      <section v-for="(config, type) in configs" :key="type" class="data-section">
        <header><h2>{{ config.title }} ({{ rows(type).length }})</h2><button class="add-button" :disabled="Boolean(form) || !periodId" @click="add(type)">Dodaj događanje</button></header>
        <div v-if="rows(type).length || (activeType === type && form)" class="records-layout">
          <div class="event-list">
            <button v-for="item in rows(type)" :key="item.id" class="event-row" :class="{ selected: activeType === type && activeId === item.id }" :disabled="Boolean(form)" @click="selectRow(type, item.id)">{{ item.event_name }}</button>
          </div>

          <dl v-if="activeType === type && (form || selectedItem(type))" class="details-card">
            <div class="details-actions">
              <template v-if="form">
                <button :disabled="saving" @click="save(type)">{{ saving ? 'Spremanje...' : 'Spremi' }}</button>
                <button :disabled="saving" @click="cancelEdit">Odustani</button>
                <button v-if="activeId !== 'new'" class="square" :disabled="saving" aria-label="Izbriši događanje" title="Izbriši događanje" @click="remove(type)">−</button>
              </template>
              <button v-else @click="edit(type)">Uredi</button>
            </div>
            <div><dt>Broj</dt><dd>{{ activeId === 'new' ? 'Novi' : selectedItem(type).id }}</dd></div>
            <div v-for="field in config.fields" :key="field.key">
              ...
              <dd>
                ...
                <a v-else-if="field.kind === 'link' && selectedItem(type)[field.key]" :href="selectedItem(type)[field.key]" target="_blank" rel="noopener">{{ selectedItem(type)[field.key] }}</a>
      ...
      <section class="analysis"><h2>Analiza prema vrsti događanja</h2><div class="analysis-wrap"><table><thead><tr><th>Vrsta događanja</th><th>Održano</th><th>Planirano</th><th>Ukupno sudionika</th><th>Prosječno sudionika</th></tr></thead><tbody><tr v-for="item in typeAnalysis" :key="item.eventType"><td>{{ item.eventType }}</td><td>{{ item.held }}</td><td>{{ item.planned }}</td><td>{{ item.participants }}</td><td>{{ item.average }}</td></tr><tr class="total"><td>Ukupno</td><td>{{ metrics.held }}</td><td>{{ metrics.planned }}</td><td>{{ metrics.participants }}</td><td>{{ metrics.held ? Math.round(metrics.participants / metrics.held) : 0 }}</td></tr></tbody></table></div></section>
    </template>
