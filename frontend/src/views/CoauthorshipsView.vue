...
const periodPapers = computed(() =>
  papers.value.filter((item) => Number(item.reporting_period_id) === Number(periodId.value)),
)
const periodSummaries = computed(() =>
  summaries.value.filter((item) => Number(item.reporting_period_id) === Number(periodId.value)),
)
...
function summaryCell(category, year) {
  return (
    periodSummaries.value.find(
      (item) => item.category === category && Number(item.calendar_year) === Number(year),
    ) || null
  )
}
...
function choosePaper(item) {
  selectedPaper.value = item
  paperForm.value = null
  editError.value = ''
}
function startPaperEdit() {
  paperForm.value = {
    authors_and_title: selectedPaper.value.authors_and_title,
    publication_year: selectedPaper.value.publication_year,
    category: selectedPaper.value.category || '',
    publication_link: selectedPaper.value.publication_link || '',
  }
}
...
async function savePaper() {
  if (!paperForm.value.authors_and_title.trim() || !paperForm.value.category) {
    editError.value = 'Autori, naslov i kategorija rada su obavezni.'
    return
  }
  saving.value = true
  try {
...
async function load() {
  try {
    ...
    error.value = apiError(exception, 'Nije moguće dohvatiti koautorstva.')
  } finally {
    loading.value = false
  }
}
    ...
    <template v-else>
      ...
      <section class="records">
        <header class="heading">
          <h2>Popis radova u koautorstvu ({{ filtered.length }})</h2>
          <RouterLink class="action" to="/istrazivanje-i-razvoj/koautorstva/novo"
            >Dodaj novi rad</RouterLink
          >
        </header>
        <div v-if="filtered.length" class="layout">
          <div>
            <div class="list">
              <button
                v-for="item in shown"
                :key="item.id"
                :class="{ selected: selectedPaper?.id === item.id }"
                @click="choosePaper(item)"
              >
                <strong>{{ item.authors_and_title }}</strong>
              </button>
            </div>
          ...
          <dl v-if="selectedPaper" ref="detailsCard" class="details">
            <div class="actions">
              <button v-if="!paperForm" class="action" @click="startPaperEdit">Uredi</button
              ><template v-else
                ><button class="action" :disabled="saving" @click="savePaper">
                  {{ saving ? 'Spremanje...' : 'Spremi' }}</button
                ><button class="action" :disabled="saving" @click="cancelPaperEdit">Odustani</button
                ><button class="square minus" :disabled="saving" @click="deletePaper">
                  −
                </button></template
              >
            </div>
            ...
            <div>
              <dt>Autori i naslov rada</dt>
              <dd>
                <textarea
                  v-if="paperForm"
                  v-model="paperForm.authors_and_title"
                  class="input"
                  rows="5"
                ></textarea
                ><template v-else>{{ selectedPaper.authors_and_title }}</template>
              </dd>
            </div>
            <div>
              <dt>Godina objave</dt>
              <dd>
                <input
                  v-if="paperForm"
                  v-model="paperForm.publication_year"
                  class="input"
                  type="number"
                  min="2000"
                  :max="new Date().getFullYear() + 1"
                /><template v-else>{{ selectedPaper.publication_year }}.</template>
              </dd>
            </div>
            <div>
              <dt>Kategorija rada</dt>
              <dd>
                <select v-if="paperForm" v-model="paperForm.category" class="input">
                ...
                ><template v-else>{{
                  selectedPaper.category ? categoryLabel(selectedPaper.category) : 'Nije određena'
                }}</template>
              </dd>
            </div>
            <div>
              <dt>Link na objavljeni rad</dt>
              <dd>
                <input v-if="paperForm" v-model="paperForm.publication_link" class="input" /><a
                  v-else-if="safeLink(selectedPaper.publication_link)"
                  :href="safeLink(selectedPaper.publication_link)"
                  target="_blank"
                  >{{ selectedPaper.publication_link }}</a
                ><template v-else>{{ display(selectedPaper.publication_link) }}</template>
              </dd>
            </div>
          </dl>
        </div>
          ...
          <table>
            <thead>
              <tr>
                ...
                <th v-if="!years.length">Broj radova</th>
              </tr>
            </thead>
            <tbody>
              <tr
                ...
                :key="category.value"
                :class="{ total: category.value === 'TOTAL' }"
              >
                <td>{{ category.label }}</td>
                <td v-for="year in years" :key="year">
                  {{ summaryCell(category.value, year)?.paper_count ?? '—' }}
                </td>
                <td v-if="!years.length">—</td>
              </tr>
            </tbody>
          </table>
        </div>
...
.layout {
  display: grid;
  grid-template-columns: minmax(380px, 1.1fr) minmax(440px, 0.9fr);
