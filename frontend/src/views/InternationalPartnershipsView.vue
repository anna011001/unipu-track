...
const filteredNew = computed(() =>
  newRecords.value.filter((item) => Number(item.reporting_period_id) === Number(periodId.value)),
)
const filteredAgreements = computed(() =>
  agreements.value.filter((item) => Number(item.reporting_period_id) === Number(periodId.value)),
)
...
const shownNew = computed(() =>
  filteredNew.value.slice((newPage.value - 1) * perPage, newPage.value * perPage),
)
const shownAgreements = computed(() =>
  filteredAgreements.value.slice(
    (agreementPage.value - 1) * perPage,
    agreementPage.value * perPage,
  ),
)
const currentFields = computed(() =>
  selected.value?.type === 'agreement' ? agreementFields : newFields,
)

const regionRows = computed(() => {
  const rows = Object.keys(regionLabels).map((region) => ({
    region,
    scientific: 0,
    artistic: 0,
    professional: 0,
  ...
  const countryRegion = new Map(
    countries.value.map((country) => [Number(country.id), country.region]),
  )
...
function choose(type, record) {
  selected.value = { type, record }
  form.value = null
  editError.value = ''
}
function startEdit() {
  form.value = Object.fromEntries(
    currentFields.value.map((field) => [
      field.key,
      field.kind === 'date'
        ? dateInput(selected.value.record[field.key])
        : (selected.value.record[field.key] ?? ''),
    ]),
  )
}
function cancelEdit() {
...
async function save() {
  editError.value = ''
  if (!String(form.value.partner_institution || '').trim()) {
    editError.value = 'Partnerska institucija je obavezna.'
    return
  }
  if (
    selected.value.type === 'agreement' &&
    form.value.signed_on &&
    form.value.valid_until &&
    form.value.valid_until < form.value.signed_on
  ) {
    editError.value = 'Rok važenja ne smije biti prije datuma potpisivanja.'
    return
  }
  saving.value = true
  try {
    ...
    const response = await api.patch(
      `/api/international-cooperations/${path}/${selected.value.record.id}`,
      editPayload(),
    )
    ...
    toast(
      selected.value.type === 'agreement'
        ? 'Međunarodni ugovor uspješno je izmijenjen.'
        : 'Partnerstvo uspješno je izmijenjeno.',
    )
  } catch (exception) {
    editError.value = apiError(exception, 'Zapis nije moguće izmijeniti.')
  } finally {
    saving.value = false
  }
}
...
async function remove() {
  ...
  try {
    ...
    if (selected.value.type === 'agreement')
      agreements.value = agreements.value.filter((item) => item.id !== selected.value.record.id)
    else newRecords.value = newRecords.value.filter((item) => item.id !== selected.value.record.id)
    selected.value = null
    form.value = null
    toast('Zapis je uspješno izbrisan.')
  } catch (exception) {
    editError.value = apiError(exception, 'Zapis nije moguće izbrisati.')
  } finally {
...
async function openFromRoute() {
  ...
  if (type === 'agreement') agreementPage.value = page
  else newPage.value = page
  choose(type, item)
  await nextTick()
...
async function load() {
  try {
    const [periodResponse, newResponse, agreementResponse, countryResponse, unitResponse] =
      await Promise.all([
        api.get('/api/reporting-periods'),
        api.get('/api/international-cooperations/new'),
        api.get('/api/international-cooperations/agreements'),
        api.get('/api/countries'),
        api.get('/api/organizational-units'),
      ])
    periods.value = periodResponse.data
    newRecords.value = newResponse.data
    agreements.value = agreementResponse.data
    countries.value = countryResponse.data
    units.value = unitResponse.data
    periodId.value = periods.value[0]?.id ?? null
    await openFromRoute()
  } catch (exception) {
    error.value = apiError(exception, 'Nije moguće dohvatiti međunarodna partnerstva.')
  } finally {
    loading.value = false
  }
}
  ...
  <main class="view">
    ...
    <template v-else>
      <section class="overview">
        ...
        <dl class="metrics">
          <div>
            <dt>Aktivna partnerstva</dt>
            <dd>{{ filteredAgreements.length }}</dd>
          </div>
          <div>
            <dt>Nova partnerstva</dt>
            <dd>{{ filteredNew.length }}</dd>
          </div>
          <div>
            <dt>Novi ugovori</dt>
            <dd>{{ filteredNew.filter((item) => item.agreement_type).length }}</dd>
          </div>
        </dl>
        <ExportButton
          :records="[
            ...filteredNew.map((item) => ({ vrsta_zapisa: 'Novo partnerstvo', ...item })),
            ...filteredAgreements.map((item) => ({ vrsta_zapisa: 'Aktivno partnerstvo', ...item })),
          ]"
          file-name="medunarodna-partnerstva"
        />
      </section>

      <section class="records">
        <header class="heading">
          <h2>Nova međunarodna partnerstva ({{ filteredNew.length }})</h2>
          <RouterLink
            class="action"
            :to="{ name: 'new-international-partnership', query: { type: 'new' } }"
            >Dodaj partnerstvo</RouterLink
          >
        </header>
        <div v-if="filteredNew.length" class="layout">
          <div>
            <div class="list">
              <button
                v-for="item in shownNew"
                :key="item.id"
                :class="{ selected: selected?.type === 'new' && selected.record.id === item.id }"
                @click="choose('new', item)"
              >
                <strong>{{ item.partner_institution }}</strong
                ><span
                  >{{ item.country_name_hr || item.country_name_en || 'Država nije navedena' }} ·
                  {{ kindLabels[item.cooperation_kind] || 'Vrsta nije navedena' }}</span
                >
              </button>
            </div>
            <div v-if="newPageCount > 1" class="pagination">
              <button
                v-for="number in newPageCount"
                :key="number"
                :class="{ active: newPage === number }"
          ...
          <div v-if="selected?.type === 'new'" ref="detailsCard" class="details">
            <div class="actions">
            ...
            <dl>
              <div v-for="field in currentFields" :key="field.key">
                ...
                <dd>
                  ...
                  ><select
                    v-else-if="form && field.kind === 'kind'"
                    v-model="form[field.key]"
                    class="input"
                  >
                    <option value="">Nije odabrano</option>
                  ...
                  /><a
                    v-else-if="field.kind === 'link' && isUrl(selected.record[field.key])"
                    :href="selected.record[field.key]"
                    target="_blank"
                    rel="noopener"
                    >{{ selected.record[field.key] }}</a
        ...
        <p v-else>Nema novih partnerstava za odabrano razdoblje.</p>
      </section>

      <section class="records">
        <header class="heading">
          <h2>Aktivni međunarodni ugovori ({{ filteredAgreements.length }})</h2>
          <RouterLink
            class="action"
            :to="{ name: 'new-international-partnership', query: { type: 'agreement' } }"
            >Dodaj ugovor</RouterLink
          >
        </header>
        <div v-if="filteredAgreements.length" class="layout">
          <div>
            <div class="list">
              <button
                v-for="item in shownAgreements"
                :key="item.id"
                :class="{
                  selected: selected?.type === 'agreement' && selected.record.id === item.id,
                }"
                @click="choose('agreement', item)"
              >
                <strong>{{ item.partner_institution }}</strong
                ><span
                  >{{ item.agreement_type || 'Vrsta ugovora nije navedena' }} ·
                  {{ date(item.signed_on) }}</span
                >
              </button>
            </div>
            <div v-if="agreementPageCount > 1" class="pagination">
              <button
                v-for="number in agreementPageCount"
                :key="number"
                :class="{ active: agreementPage === number }"
                @click="agreementPage = number"
              >
                {{ number }}
              </button>
            </div>
          </div>
          <div v-if="selected?.type === 'agreement'" ref="detailsCard" class="details">
            <div class="actions">
              <button v-if="!form" class="action" @click="startEdit">Uredi</button
              ><template v-else
                ><button class="action" :disabled="saving || deleting" @click="save">
        ...
        <p v-else>Nema aktivnih ugovora za odabrano razdoblje.</p>
      </section>

      <section class="summary">
        <h2>Analiza međunarodne suradnje po regijama</h2>
        <div class="table">
          <table>
            <thead>
              <tr>
                ...
                <th>Umjetnička</th>
                <th>Profesionalna</th>
                <th>Ukupno aktivnih</th>
                <th>Nova partnerstva</th>
              </tr>
            </thead>
            <tbody>
              ...
              <tr class="total">
                <td>Ukupno</td>
                <td>{{ regionTotals.scientific }}</td>
                <td>{{ regionTotals.artistic }}</td>
                <td>{{ regionTotals.professional }}</td>
                <td>{{ regionTotals.total }}</td>
                <td>{{ regionTotals.new }}</td>
              </tr>
            </tbody>
          </table>
        </div>
