...
<template>
  <main class="view">
    ...
    <template v-else>
      <section class="overview"><label>Izvještajno razdoblje<select v-model.number="periodId"><option v-for="period in periods" :key="period.id" :value="period.id">{{ period.label }}</option></select></label><dl class="metrics"><div><dt>Aktivna partnerstva</dt><dd>{{ filteredAgreements.length }}</dd></div><div><dt>Nova partnerstva</dt><dd>{{ filteredNew.length }}</dd></div><div><dt>Novi ugovori</dt><dd>{{ filteredNew.filter((item) => item.agreement_type).length }}</dd></div></dl><ExportButton :records="[...filteredNew.map((item) => ({ vrsta_zapisa: 'Novo partnerstvo', ...item })), ...filteredAgreements.map((item) => ({ vrsta_zapisa: 'Aktivno partnerstvo', ...item }))]" file-name="medunarodna-partnerstva" /></section>
