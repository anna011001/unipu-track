...
const selectedUnit = computed(() =>
  units.value.find((item) => Number(item.id) === Number(unitId.value)),
)
const academicYear = computed(() => selectedPeriod.value?.label || '20../20..')
const linkedKeys = computed(() => [
  ...new Set(reportSections.flatMap((section) => section.links || [])),
])
...
function makeDraft(item = {}) {
  return {
    ...
    ...Object.fromEntries(
      reportSections.map((section) => [section.narrative, item[section.narrative] || '']),
    ),
  }
}
    ...
    const payload = {
      ...
      ...Object.fromEntries(
        reportSections.map((section) => [
          section.narrative,
          draft.value[section.narrative].trim() || null,
        ]),
      ),
      updated_by: userId,
    }
...
async function loadReportData() {
  ...
  const results = await Promise.allSettled(
    allTableConfigs.map((config) => api.get(`/api/faculty/${config.endpoint}`)),
  )
  const rows = {}
    ...
    rows[endpoint] =
      result.status === 'fulfilled'
        ? result.value.data.filter(
            (item) => Number(item.faculty_report_id) === Number(report.value.id),
          )
        : []
  })
...
async function loadLinkedEvidence() {
  const results = await Promise.allSettled(
    linkedKeys.value.map((key) => api.get(linkedEvidence[key].endpoint)),
  )
  const records = Object.fromEntries(
    results.map((result, index) => {
      ...
      const filtered = data.filter(
        (item) =>
          item.reporting_period_id === undefined ||
          Number(item.reporting_period_id) === Number(periodId.value),
      )
      return [key, filtered]
    }),
  ...
  linkedCounts.value = Object.fromEntries(
    linkedKeys.value.map((key) => [key, records[key]?.length ?? 0]),
  )
}
    ...
    <template v-else>
      <div class="report-toolbar no-print">
        ...
        <div class="toolbar-actions">
          <button v-if="report" type="button" :disabled="saving" @click="saveReport">
            {{ saving ? 'Spremanje…' : 'Spremi obrazac' }}
          </button>
          <button v-if="report" type="button" :disabled="syncing" @click="loadReportData">
            {{ syncing ? 'Osvježavanje…' : 'Osvježi povezane podatke' }}
          </button>
          <button v-if="report" type="button" @click="exportPdf">Izvoz PDF</button>
          <button v-else type="button" :disabled="saving" @click="createReport">
            Napravi obrazac
          </button>
        </div>
      </div>
      ...
      <div v-if="!report" class="empty-report no-print">
        ...
        <p>
          Za odabrano razdoblje i sastavnicu još nema izvještaja. Napravite ga kako biste mogli
          unositi tekst, uređivati tablice i povlačiti povezane evidencije.
        </p>
      </div>
      ...
      <div v-else class="report-workspace">
        <aside :class="['report-sidebar', 'no-print', { 'is-open': sidebarOpen }]">
          ...
          >
            ...
            >
              <path
                class="sidebar-wave-fill"
                d="M 1 0 L 1 12 C 1 25 18 25 18 37 C 18 49 1 49 1 62 L 1 74 L 0 74 L 0 0 Z"
              />
              <path
                class="sidebar-wave-line"
                d="M 1 0 L 1 12 C 1 25 18 25 18 37 C 18 49 1 49 1 62 L 1 74"
              />
            </svg>
        ...
        <article class="report-document">
          <section id="report-cover" class="report-cover print-page">
            ...
            <div class="cover-title">
              ...
              <strong class="dean-print-name print-only">{{
                draft.dean_name || 'Ime i prezime dekana'
              }}</strong>
            </div>
            ...
            <p class="cover-date">
              ...
              {{
                draft.report_date
                  ? new Intl.DateTimeFormat('hr-HR').format(new Date(draft.report_date))
                  : '__. __. 20__.'
              }}
            </p>
          </section>
          ...
          <section class="submission-page print-page">
            <p>
              Sukladno čl. 42. st. 5. Statuta Sveučilišta podnosim Senatu izvješće o radu i
          ...
          <section id="report-intro" class="intro-page print-page">
            ...
            <p>
              Ovo Izvješće o radu i poslovanju fakulteta odnosno akademije izrađeno je u svrhu
              sustavnog praćenja provedbe strateških ciljeva Sveučilišta Jurja Dobrile u Puli te
              objedinjuje pokazatelje i aktivnosti koji proizlaze iz:
            </p>
            <ul>
            ...
            <p>
              Cilj ovako strukturiranog obrasca jest objediniti podatke na jednom mjestu kako bi se
              izbjeglo višestruko traženje istih informacija tijekom akademske godine te osigurala
              usporedivost i preglednost podataka između sastavnica.
            </p>
            <p>
              Izvješće o radu i poslovanju fakulteta odnosno akademije za prethodnu akademsku godinu
              potrebno je dostaviti najkasnije do <strong>30. listopada tekuće godine.</strong>
            </p>
            <p>
              Nakon podnošenja pisanog Izvješća, dekan je obvezan isto prezentirati Senatu
              Sveučilišta, s posebnim naglaskom na ključne rezultate, izazove i planirane aktivnosti
          ...
          <section id="report-contents" class="contents-page print-page">
            ...
            <ul class="contents-list">
              <li v-for="section in reportSections" :key="section.number">
                <a :href="`#section-${section.narrative}`"
                  ><span>{{ section.number }}</span> {{ section.title }}</a
                >
              </li>
            </ul>
          </section>
          ...
          >
            ...
            <label class="narrative-field">
              <textarea
                v-model="draft[section.narrative]"
                class="no-print"
                :maxlength="section.limit"
                rows="8"
              />
              <small class="no-print"
                >{{ draft[section.narrative]?.length || 0 }} / {{ section.limit }}</small
              >
              <p class="narrative-print print-only">
                {{ draft[section.narrative] || 'Nije uneseno.' }}
              </p>
            </label>

            <template
              v-for="(block, blockIndex) in sectionContent(section)"
              :key="`${block.type}-${block.endpoint || block.keys?.join('-')}-${blockIndex}`"
            >
              <div v-if="block.type === 'evidence'" class="linked-evidence">
                ...
                <div class="linked-grid no-print">
                  ...
                  >
                    ...
                    <strong>{{
                      linkedCounts[key] === null || linkedCounts[key] === undefined
                        ? '—'
                        : linkedCounts[key]
                    }}</strong>
                  </RouterLink>
...
.faculty-report-view {
  ...
  padding: 42px clamp(24px, 5vw, 96px) 90px;
...
.sidebar-panel {
  ...
  background: rgb(var(--v-theme-surface));
  box-shadow: 7px 5px 24px rgba(0, 0, 0, 0.14);
}
.report-sidebar h2 {
  margin: 0 10px 14px;
  font-size: 1.15rem;
...
.sidebar-divider {
  height: 1px;
  margin: 8px 10px;
  background: rgb(var(--v-theme-table-border));
}
.sidebar-handle {
  position: absolute;
  top: 50%;
...
@media (min-width: 600px) {
  ...
  .report-sidebar:focus-within,
  .report-sidebar.is-open {
    transform: translate(0, -50%);
  }
  .report-sidebar:hover .sidebar-handle span,
  .report-sidebar:focus-within .sidebar-handle span,
  .report-sidebar.is-open .sidebar-handle span {
