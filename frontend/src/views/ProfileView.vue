...
async function loadStaffProfile() {
  try {
    ...
    staffProfile.value = response.data.find((member) => (
    ...
    <section class="profile-card">
      ...
      <form @submit.prevent="changePassword">
        ...
        <label>
          ...
          <input v-model="form.current_password" type="password" autocomplete="current-password" :disabled="saving">
        </label>
        <label>
          <span>Nova lozinka</span>
          <input v-model="form.new_password" type="password" autocomplete="new-password" :disabled="saving">
        </label>
        <label>
          <span>Ponovite novu lozinku</span>
          <input v-model="form.password_confirmation" type="password" autocomplete="new-password" :disabled="saving">
        </label>

        <p v-if="error" class="message error" role="alert">{{ error }}</p>
        <p v-if="success" class="message success" role="status">{{ success }}</p>
        <button type="submit" :disabled="saving">{{ saving ? 'Spremanje...' : 'Promijeni lozinku' }}</button>
      </form>
    </section>
  </main>
</template>
