    ...
    <template v-else>
      ...
      <section class="records">
        <header class="heading">
          ...
          <RouterLink class="action" to="/istrazivanje-i-razvoj/projektne-prijave/nova"
            >Dodaj novu prijavu</RouterLink
          >
        </header>
        ...
        <div v-if="filtered.length" class="layout">
          ...
          <dl v-if="selected" ref="detailsCard" class="details">
            ...
            <div>
              <dt>Naziv projektnog prijedloga</dt>
              <dd>
                <textarea v-if="form" v-model="form.proposal_name" class="input" rows="3"></textarea
                ><template v-else>{{ selected.proposal_name }}</template>
              </dd>
            </div>
            <div>
              <dt>Izvor financiranja</dt>
              <dd>
                <input v-if="form" v-model="form.funding_source" class="input" /><template v-else>{{
                  display(selected.funding_source)
                }}</template>
              </dd>
            </div>
            <div>
              <dt>Poziv / natječaj</dt>
              <dd>
                <input v-if="form" v-model="form.call_name" class="input" /><template v-else>{{
                  display(selected.call_name)
                }}</template>
              </dd>
            </div>
            <div>
              <dt>Poveznica na poziv</dt>
              <dd>
                <input v-if="form" v-model="form.call_link" class="input" type="url" /><a
                  v-else-if="safeLink(selected.call_link)"
                  :href="safeLink(selected.call_link)"
                  target="_blank"
                  rel="noopener"
                  >{{ selected.call_link }}</a
                ><template v-else>{{ display(selected.call_link) }}</template>
              </dd>
            </div>
            <div>
              <dt>Uloga UNIPU-a</dt>
              <dd>
                <input v-if="form" v-model="form.unipu_role" class="input" /><template v-else>{{
                  display(selected.unipu_role)
                }}</template>
              </dd>
            </div>
            <div>
              <dt>Uključene sastavnice</dt>
              <dd>
                <textarea
                  v-if="form"
                  ...
                  rows="3"
                ></textarea
                ><template v-else>{{ display(selected.involved_units) }}</template>
              </dd>
            </div>
            <div>
              <dt>Partnerske institucije</dt>
              <dd>
                <textarea
                  v-if="form"
                  ...
                  rows="3"
                ></textarea
                ><template v-else>{{ display(selected.partner_institutions) }}</template>
              </dd>
            </div>
            <div>
              <dt>Ukupni iznos projekta</dt>
              <dd>
                <input
                  v-if="form"
                  ...
                  max="999999.99"
                  step="0.01"
                /><template v-else>{{ formatMoney(selected.total_project_amount_eur) }}</template>
              </dd>
            </div>
            <div>
              <dt>Udio UNIPU-a</dt>
              <dd>
                <input
                  ...
                  max="999999.99"
                  step="0.01"
                /><template v-else>{{ formatMoney(selected.unipu_share_eur) }}</template>
              </dd>
            </div>
            <div>
              <dt>Trajanje provedbe</dt>
              <dd>
                <input v-if="form" v-model="form.implementation_duration" class="input" /><template
                  v-else
                  >{{ display(selected.implementation_duration) }}</template
                >
              </dd>
            </div>
            <div>
              <dt>Vrsta projekta</dt>
              <dd>
                <select v-if="form" v-model="form.project_type" class="input">
                  <option value="">Nije odabrano</option>
                  <option v-for="item in projectTypes" :key="item.value" :value="item.value">
                    {{ item.label }}
                  </option></select
                ><template v-else>{{ labelFor(projectTypes, selected.project_type) }}</template>
              </dd>
            </div>
            <div>
              <dt>Planirane aktivnosti</dt>
              <dd>
                <textarea
                  v-if="form"
                  ...
                  rows="4"
                ></textarea
                ><template v-else>{{ display(selected.planned_activities) }}</template>
              </dd>
            </div>
            <div>
              <dt>Projektni tim UNIPU-a</dt>
              <dd>
                <textarea
                  v-if="form"
                  ...
                  rows="3"
                ></textarea
                ><template v-else>{{ display(selected.unipu_project_team) }}</template>
              </dd>
            </div>
            <div>
              <dt>Rok za prijavu</dt>
              <dd>
                <input
                  v-if="form"
                  ...
                  class="input"
                  type="date"
                /><template v-else>{{ formatDate(selected.submission_deadline) }}</template>
              </dd>
            </div>
            <div>
              <dt>Status prijave</dt>
              <dd>
                <select v-if="form" v-model="form.application_status" class="input">
                  <option value="">Nije odabrano</option>
                  <option v-for="item in statuses" :key="item.value" :value="item.value">
                    {{ item.label }}
                  </option></select
                ><template v-else>{{ labelFor(statuses, selected.application_status) }}</template>
              </dd>
            </div>
            <div>
              <dt>Ugovor ili sporazum o partnerstvu</dt>
              <dd>
                <input
                  v-if="form"
                  ...
                  class="input"
                /><template v-else>{{
                  display(selected.contract_or_partnership_reference)
                }}</template>
              </dd>
            </div>
            <div>
              <dt>Šifra projekta po ugovoru</dt>
              <dd>
                <input v-if="form" v-model="form.contract_project_code" class="input" /><template
                  v-else
                  >{{ display(selected.contract_project_code) }}</template
                >
              </dd>
            </div>
            <div>
              <dt>Napomena</dt>
              <dd>
                <textarea v-if="form" v-model="form.notes" class="input" rows="4"></textarea
