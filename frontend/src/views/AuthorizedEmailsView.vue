...
async function removeSelected() {
  ...
  const confirmed = window.confirm(`Želite li ukloniti ${entry.email} s popisa dopuštenih korisnika? Povezani račun bit će deaktiviran.`)
  if (!confirmed) return
  ...
  <main class="authorized-users-view">
    <div class="page-heading">
      <div>
        ...
        <p>Registrirati se mogu samo profesori čija se službena e-mail adresa nalazi na ovom popisu.</p>
      </div>
    </div>
    ...
    <section class="add-user-card">
      ...
      <p>Nakon dodavanja korisnik se može odmah registrirati. Ponovno dodavanje ranije uklonjene adrese aktivira postojeći račun.</p>
    </section>

    <section class="users-section">
      <div class="section-heading">
        <h2>Dopuštene e-mail adrese ({{ entries.length }})</h2>
        <button class="remove-button" type="button" :disabled="!canRemove || saving" title="Ukloni odabranu adresu" @click="removeSelected">−</button>
      </div>
      ...
      <div class="users-table">
        ...
        >
          ...
          <span data-label="Korisnik">{{ entry.is_registered ? `${entry.first_name} ${entry.last_name}` : '—' }}</span>
          <span data-label="Status" class="status" :class="entry.is_registered ? 'active' : 'waiting'">
