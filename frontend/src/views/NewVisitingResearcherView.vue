...
const title = computed(() =>
  visitType.value === 'realized'
    ? 'Dodavanje realiziranog gostovanja'
    : 'Dodavanje planiranog gostovanja',
)
...
function validate() {
  ...
  if (visitType.value === 'realized') {
    if (form.arrival_date && form.departure_date && form.departure_date < form.arrival_date)
      result.push('Datum odlaska ne smije biti prije datuma dolaska.')
    if (
      form.duration_days !== '' &&
...
async function submit() {
  ...
  try {
    ...
    success.value =
      visitType.value === 'realized'
        ? 'Realizirano gostovanje uspješno je spremljeno.'
        : 'Planirano gostovanje uspješno je spremljeno.'
  } catch (exception) {
    errors.value = exception.response?.data?.errors || [
      exception.response?.data?.message || 'Gostovanje nije moguće spremiti.',
    ]
  } finally {
    saving.value = false
  }
...
<template>
  <main class="new-view">
    <nav>
      ...
      ><RouterLink to="/medunarodna-suradnja/gostujuci-istrazivaci"
        >Gostujući istraživači</RouterLink
      ><span>›</span><span>Novo gostovanje</span>
    </nav>
    <h1>{{ title }}</h1>
    ...
    <form v-else @submit.prevent="submit">
      ...
      <div class="card">
        <label class="type-field"
          >Vrsta gostovanja<select v-model="visitType" :disabled="saved">
            <option value="realized">Realizirano gostovanje</option>
            <option value="planned">Planirano gostovanje</option>
          </select></label
        >
        <div class="grid">
          <label
            >Ime i prezime istraživača *<input
              v-model="form.researcher_name"
              maxlength="120"
              :disabled="saved"
          /></label>
          <label
            >Zvanje<input v-model="form.academic_title" maxlength="30" :disabled="saved"
          /></label>
          <label
            >Matična institucija<input
              v-model="form.home_institution"
              maxlength="200"
              :disabled="saved"
          /></label>
          <label
          ...
          /></label>
          <label
            >Sastavnica domaćin<select v-model="form.host_unit_id" :disabled="saved">
        ...
        <template v-if="visitType === 'realized'">
          <div class="grid three">
            <label
              >Datum dolaska<input v-model="form.arrival_date" type="date" :disabled="saved"
            /></label>
            <label
              >Datum odlaska<input v-model="form.departure_date" type="date" :disabled="saved"
            /></label>
            <label
              >Trajanje (dana)<input
          ...
          <label
            >Mentor / kontakt<input v-model="form.mentor_contact" maxlength="150" :disabled="saved"
          /></label>
          <div class="grid">
            <label
              >Aktivnosti tijekom boravka<textarea
                v-model="form.activities_during_stay"
                rows="4"
                :disabled="saved"
              ></textarea>
            </label>
            <label
              >Rezultati (publikacije, predavanja... )<textarea
          ...
          <div class="grid three">
            <label
              >Broj predavanja<input
                v-model="form.lecture_count"
                type="number"
                min="0"
                max="9999"
                :disabled="saved"
            /></label>
            <label
              >Broj publikacija<input
                v-model="form.publication_count"
                type="number"
                min="0"
                max="9999"
                :disabled="saved"
            /></label>
            <label
              >Broj projekata<input
                v-model="form.project_count"
                type="number"
                min="0"
                max="9999"
                :disabled="saved"
        ...
        <template v-else>
          <div class="grid">
            <label
              >Planirano razdoblje<input
                v-model="form.planned_period"
                maxlength="100"
                placeholder="npr. listopad 2026."
                :disabled="saved"
            /></label>
            <label
              >Trajanje<input
                ...
                maxlength="60"
                placeholder="npr. 14 dana"
                :disabled="saved"
            /></label>
            <label>Mentor<input v-model="form.mentor" maxlength="120" :disabled="saved" /></label>
            <label
              >Status poziva<input
                v-model="form.invitation_status"
                maxlength="40"
                :disabled="saved"
            /></label>
            <label
              >Izvor financiranja<input
      ...
      <button
        type="button"
        class="button back"
        @click="router.push('/medunarodna-suradnja/gostujuci-istrazivaci')"
      >
        Natrag
      </button>
    </form>
